...
GPA: 3.45`;
  ...
  function parseTranscript() {
    ...
    // Enhanced parser for course codes with credits and grades
    ...
    lines.forEach(line => {
      ...
      if (match) {
        const [, code, title, credits, grade] = match;
    ...
    // Fallback to simple course code extraction
    if (courses.length === 0) {
      const simplePattern = /([A-Z]{2,4}\s*\d{4})/g;
      const matches = transcriptText.match(simplePattern) || [];
      const simpleCourses = [...new Set(matches.map(c => c.replace(/\s+/g, ' ').toUpperCase()))];
      simpleCourses.forEach(code => {
        courses.push({ code, title: 'Course Title', credits: 3, grade: 'N/A' });
      });
    }
